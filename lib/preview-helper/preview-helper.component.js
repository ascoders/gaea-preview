...
var PreviewHelper = function (_React$Component) {
    ...
    (0, _createClass3.default)(PreviewHelper, [{
        ...
        value: function runEvent(eventData) {
            var effect = this.componentInfo.props.gaeaEvent && this.componentInfo.props.gaeaEvent.effects[eventData.eventIndex];
            switch (eventData.event) {
                case 'call':
                    var callData = eventData.eventData;
        ...
        value: function getSelfFunctionMap() {
            ...
            this.eventData && this.eventData.forEach(function (data) {
                if (data.typeIndex > -1 && _this4.componentInfo.props.gaeaEvent.triggers[data.typeIndex].selfCallback) {
        ...
        value: function render() {
            ...
            Object.keys(props).forEach(function (propsField) {
                if (propsField.startsWith('gaea')) {
                    return;
                }
                try {
                    props[propsField] = props[propsField].replace(/\$\{(.*)\}/g, function (str, match) {
                        return _.get(_this5.props.gaeaData, match);
                    });
                } catch (err) {}
            });
